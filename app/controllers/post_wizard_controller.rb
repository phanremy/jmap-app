...
class PostWizardController < ApplicationController
  ...
  def update
    return render_error unless @step_manager.possible_step?(@step)

    set_form

    if @form.submit(params) && @form.complete?
      ...
      render_flash
      # redirect_to post_wizard_path(id: @step_manager.current_step, post_id: @post.id)
    end
  end
  ...
  def initialize_variables
    ...
    @step_manager = Manager.new(post: @post)
  ...
  def set_form
    @form ||= @step_manager.form_for(@step).new(post: @post)
  end
